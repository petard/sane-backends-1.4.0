...
typedef enum
{
	CS3_INFRARED_OFF = 0,	/* no infrared channel */
	CS3_INFRARED_IN,	/* reading RGBI, buffering the infrared plane */
	CS3_INFRARED_OUT	/* delivering the buffered infrared plane */
}
cs3_infrared_t;
...
typedef struct
{
	...
	int samples_per_scan, depth, real_depth, bytes_per_pixel, shift_bits,
		n_colors, n_colors_out;

	/* infrared handling: a scan with infrared on is delivered as two
	   standard frames -- an RGB frame followed by a grey infrared frame.
	   n_colors is the number of channels read from the scanner (3 or 4),
	   n_colors_out is the number of channels in the RGB output frame (3). */
	cs3_infrared_t infrared_stage, infrared_next;
	...
	*h = (SANE_Handle) s;
	...
	for (i_option = 0; i_option < CS3_N_OPTIONS; i_option++) {
		...
		case CS3_OPTION_FRAME_BASE:
			...
			o.title = "Frame base offset";
			o.desc = "Constant Y shift applied to every frame to align the holder origin";
			o.type = SANE_TYPE_FIXED;
			o.unit = SANE_UNIT_MM;
			o.size = WSIZE;
			o.cap = SANE_CAP_SOFT_SELECT | SANE_CAP_SOFT_DETECT
	...
	s->infrared_stage = CS3_INFRARED_OFF;
	s->infrared_next = CS3_INFRARED_OFF;
	s->infrared_buf = NULL;
	s->n_infrared_buf = 0;
	s->infrared_index = 0;
	s->n_colors = 3;
		    ...
		    SANE_Int * i)
{
	...
	case SANE_ACTION_SET_VALUE:
		...
		case CS3_OPTION_ADAPTER:
			...
			break;
		case CS3_OPTION_FRAME_BASE:
			s->frame_base = SANE_UNFIX(*(SANE_Word *) v);
			flags |= SANE_INFO_RELOAD_PARAMS;
			break;
		case CS3_OPTION_RES:
			s->res = *(SANE_Word *) v;
...
sane_get_parameters(SANE_Handle h, SANE_Parameters * p)
{
	...
	/* An infrared scan is delivered as two standard frames: an RGB frame
	   (stage IN) followed by a grey infrared frame (stage OUT).  Both are
	   complete frames as far as the frontend is concerned. */
	if (s->infrared_stage == CS3_INFRARED_OUT) {
		p->format = SANE_FRAME_GRAY;
		p->bytes_per_line = s->logical_width * s->bytes_per_pixel;
	} else {
		p->format = SANE_FRAME_RGB;	/* XXXXXXXX CCCCCCCCCC */
		p->bytes_per_line =
			s->n_colors_out * s->logical_width * s->bytes_per_pixel;
	}
...
sane_start(SANE_Handle h)
{
	...
	/* The infrared frame (OUT stage) is served from the buffer captured
	   during the preceding RGB (IN) scan -- no new hardware scan. */
	if (s->infrared_stage == CS3_INFRARED_OUT)
		return SANE_STATUS_GOOD;
...
sane_read(SANE_Handle h, SANE_Byte * buf, SANE_Int maxlen, SANE_Int * len)
{
	...
	/* infrared output stage: serve the buffered infrared plane as a
	   separate grey frame */
	if (s->infrared_stage == CS3_INFRARED_OUT) {
		xfer_len_out = maxlen;
		...
		if (xfer_len_out <= 0) {	/* infrared frame complete */
			*len = 0;
			/* the physical frame is fully delivered (RGB + IR);
			   advance to the next frame if batch scanning */
			if (s->n_frames > 1 && --s->frame_count)
				s->i_frame++;
			/* a subsequent frame starts a fresh RGB+IR pair */
			s->infrared_next = CS3_INFRARED_IN;
			s->scanning = SANE_FALSE;
	...
	/* output is the RGB frame (n_colors_out channels); input from the
	   scanner has n_colors channels (4 when reading infrared) */
	xfer_len_line = s->n_colors_out * s->logical_width * s->bytes_per_pixel;
	xfer_len_in = s->n_colors * s->logical_width * s->bytes_per_pixel
		+ (s->n_colors * s->odd_padding);
	...
	if (xfer_len_line == 0) {	/* no more data */
		*len = 0;
		...
		if (s->infrared_stage == CS3_INFRARED_IN) {
			/* the RGB frame is complete; the infrared frame for the
			   same physical frame is delivered next.  Do not advance
			   the physical frame yet. */
			s->infrared_next = CS3_INFRARED_OUT;
		} else if (s->n_frames > 1 && --s->frame_count) {
	...
	for (index = 0; index < s->logical_width; index++) {
		for (color = 0; color < s->n_colors; color++) {
			/* the infrared plane (4th channel) is split off into
			   its own buffer; RGB goes into the output line */
			int to_infrared = (s->infrared_stage == CS3_INFRARED_IN
					   && color == 3);
			int where = s->bytes_per_pixel
				* (s->n_colors_out * index + color);
			...
			case 2:
			{
				/* target address */
				if (to_infrared)
					s16 = (uint16_t *) & (s->infrared_buf[2 * (s->infrared_index++)]);
				else
...
sane_cancel(SANE_Handle h)
{
	...
	/* no hardware scan is running during the infrared output stage */
	if (s->scanning && (s->infrared_stage != CS3_INFRARED_OUT)) {
		cs3_init_buffer(s);
...
cs3_convert_options(cs3_t * s)
{
	...
	if (s->focus_on_centre) {
		...
		s->real_focusy =
			s->focusy + s->frame_base / s->unit_mm +
			(s->i_frame - 1) * s->frame_offset +
			s->subframe / s->unit_mm;
	}
	...
	/* The RGB output frame always has 3 channels.  xfer_bytes_total
	   accounts for that RGB frame; the infrared plane is buffered
	   separately (n_infrared_buf). */
	s->n_colors_out = 3;	/* XXXXXXXXXXXXXX CCCCCCCCCCCCCC */
	s->n_colors = 3;

	s->xfer_bytes_total =
		s->bytes_per_pixel * s->n_colors_out * s->logical_width *
		s->logical_height;

	if (!s->infrared) {
		s->infrared_stage = s->infrared_next = CS3_INFRARED_OFF;
	} else {
		SANE_Byte *infrared_buf_new;

		/* schedule the infrared (IN) stage when starting fresh */
		if (s->infrared_stage == CS3_INFRARED_OFF)
			s->infrared_next = CS3_INFRARED_IN;
		s->infrared_stage = s->infrared_next;

		/* read the 4th (infrared) channel from the scanner */
		s->n_colors = 4;

		s->n_infrared_buf = s->bytes_per_pixel * s->logical_width *
			s->logical_height;
		infrared_buf_new = (SANE_Byte *) cs3_xrealloc(s->infrared_buf,
							      s->n_infrared_buf);
		if (!infrared_buf_new)
			return SANE_STATUS_NO_MEM;
